...
const ChallengePage = ({ user }) => {
  ...
  const handleShareChallenge = () => {
    ...
    const challengeLink = `${window.location.origin}/challenge?score=${userScore}&inviter=${encodeURIComponent(currentUser)}`;

    const whatsappMessage = `Join me in the Globetrotter Challenge! 🌍\n\n${currentUser} scored ${userScore} points!\nCan you beat this score?\n\n${challengeLink}`;
    ...
    navigator.clipboard.writeText(challengeLink).then(() => {
      alert('Link copied! You can paste it anywhere to invite your friend.');
    }).catch(() => {
      alert('Link ready to share: ' + challengeLink);
    });
  };
